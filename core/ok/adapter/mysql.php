 ...
 * @package database
 */
class OK_Adapter_Mysql extends OK_Adapter_Abstract
{
	/**
	 * 设置 USE 当前数据库
	 * 
    ...
    function connect($pconnect = false, $force_new = false)
    {
        ...
        {
        	//屏蔽  php 5.5 警告使用 mysql 扩展的信息
            $this->_conn = @mysql_connect($host, $this->_dsn['login'], $this->_dsn['password'], $force_new);
        }

        if (! is_resource($this->_conn))
        {
            throw new OK_DB_Exception('CONNECT DATABASE', mysql_error(), mysql_errno());
        }

		if(!isset(self::$_is_db_use[(string)$this->_conn])) self::$_is_db_use[(string)$this->_conn] = NULL;
        
        if (! empty($this->_dsn['database']) && self::$_is_db_use[(string)$this->_conn] != $this->_dsn['database'])
        {
            $this->_execute('USE ' . $this->qid($this->_dsn['database']));
            self::$_is_db_use[(string)$this->_conn] = $this->_dsn['database']; 
        }
    ...
    function close()
    {
        ...
        {
        	//修正长脚本内存泄漏的问题
        	unset(self::$_is_db_use[(string)$this->_conn]);
    ...
    function execute($sql, $inputarr = null)
    {
        ...
        //写日志
        if ($this->_log_enabled) Helper_DbLog::sql($this->_dsn['database'] ,str_replace("\n",' ',$sql));
    	...
    	//! 检查是否需要重新 USE db_name;
        if (self::$_is_db_use[(string)$this->_conn] != $this->_dsn['database']){
	        if (! empty($this->_dsn['database']))
	        {
	        	$this->_execute('USE ' . $this->qid($this->_dsn['database']));
                $this->_execute("SET NAMES '" . $this->_dsn['charset'] . "'");
	        }
	         self::$_is_db_use[(string)$this->_conn] = $this->_dsn['database'];
    	}
    ...
    {
    	if (OK::ini('db_sql_debug')) {
    		pprint($sql);
    	}
    	
    	if ($this->_log_enabled) $executeTime = microtime(1);
    	
        $result = mysql_query($sql, $this->_conn);
        
        //计算执行SQL时间
        if ($this->_log_enabled) Helper_DbLog::time(microtime(1) - $executeTime);
