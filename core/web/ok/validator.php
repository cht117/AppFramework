...
/**
 * OK_Validator 提供了一组验证方法，以及调用验证方法的接口
 *
 * @package helper
 */
abstract class OK_Validator
{
    // 当有规则失败时，跳过余下的规则
     ...
     *
     * @code php
     * if (!OK_Validator::validate($value, 'max', 5))
     * {
     *     echo 'value 不能大于 5';
     * }
     * @endcode
     *
     * $validation 参数是验证规则。
     * 如果使用 OK_Validator 自带的验证规则，可以使用如下的写法：
     *
     * @code php
     * OK_Validator::validate($value, 'between', 1, 5);
     * @endcode
     *
     * 'between' 是验证规则的名字，对应于 OK_Validator::validate_between() 方法。
     * 因此要使用 OK_Validator 的其他验证方法，将方法名中的“validate_”去掉即为验证规则名。
     *
     * 如果要使用自定义的验证规则，有三种写法：
     *
     * @code php
     * // 使用某个类的静态方法做验证方法
     * OK_Validator::validate($value, array('MyClass', 'myMethod'), $args);
     *
     * // 功能同上
     * OK_Validator::validate($value, 'MyClass::myMethod', $args);
     *
     * // 使用某个对象的方法做验证方法
     * OK_Validator::validate($value, array($my_obj, 'myMethod), $args);
     * @endcode
     *
     ...
     *
     * @code php
     * $ret = OK_Validator::validateBatch($value, array(
     *         array('is_int'),
     *         array('between', 2, 6),
     * ));
     * @endcode
     ...
     * @code php
     * $failed = null;
     * $ret = OK_Validator::validateBatch($value, $validations, true, $failed);
     *
     ...
     * @code php
     * // validate() 的写法
     * OK_Validator::validate($value, 'max', 6);
     *
     * // validateByArgs() 的写法
     * OK_Validator::validateByArgs('max', array($value, 6));
     * @endcode
     *
    ...
    static function validateByArgs($validation, array $args)
    {
        ...
        {
            throw new OK_NotImplementedException(__($validation));
        }
        return $result;
    }
