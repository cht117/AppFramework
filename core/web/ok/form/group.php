...
/**
 * 类 OK_Form_Group 是容纳多个元素或群组的集合
 *
 * @package form
 */
class OK_Form_Group extends OK_Form_Element implements ArrayAccess
{
    /**
     * 聚合的元素
     *
     ...
     */
    function __construct($id = null, array $attrs = null, OK_Form_Group $owner = null)
    {
        parent::__construct($id, $attrs, $owner);
        $this->_elements = new OK_Coll('OK_Form_Element');
    }
     ...
     *
     * @code php
     * $form->add(OK_Form::ELEMENT, 'title', array('_ui' => 'textbox', 'size' => 40));
     * @endcode
     *
     * $type 参数只能是 OK_Form::ELEMENT 或 OK_Form::GROUP 两者之一。
     *
    ...
    {
        if ($type == OK_Form::ELEMENT)
        {
            $item = new OK_Form_Element($id, $attrs, $this);
        }
        elseif ($type == OK_Form::GROUP)
        {
            $item = new OK_Form_Group($id, null, $this);
            if (!empty($attrs))
            {
        ...
        {
            throw new OK_Form_Exception(__('Invalid type "%s".', $type));
        }
    ...
    function loadFromConfig(array $config)
    {
        foreach ($config as $id => $attrs)
        {
            ...
            if(!isset($this->_attrs['ok_form_group_id']))
            {
                $this->_attrs['ok_form_group_id'] = "";
            }

            if (isset($attrs['_elements']))
            {
                if(!isset($attrs['ok_form_group_id']))
                {
                    $attrs['ok_form_group_id'] = $id;
                }
                $elements = (array)$attrs['_elements'];
                unset($attrs['_elements']);
                $group = new OK_Form_Group($id, $attrs, $this);
                if (!empty($elements))
                {
            ...
            else
            {
                ...
                $attrs['ok_form_group_id'] = $this->_attrs['ok_form_group_id'];

                $element = new OK_Form_Element($id, $attrs, $this);
     ...
     *
     * @return OK_Form_Group 返回群组对象本身，实现连贯接口
     */
    function addValidations($source)
    {
        if ($source instanceof OK_ActiveRecord_Meta || $source instanceof OK_ActiveRecord_Meta)
        {
        ...
        {
            throw new OK_Form_Exception(__('Typemismatch, expected array, actual is "%s".', gettype($source)));
        }
    ...
    function invalidate($error = null)
    {
        ...
        if ($error instanceof OK_Validator_ValidateFailedException)
        {
            $errors = $error->validate_errors;
        }
        elseif (!is_array($error))
        {
            $keys = OK::normalize($error);
            $errors = array();
    ...
    protected function _import($data, $parent_data = null)
    {
        if (!is_array($data) && !($data instanceof ArrayAccess))
        {
            ...
            throw new OK_Form_Exception(__('$data 参数应该是一个数组或一个实现了 ArrayAccess 接口的对象.'));
        }

        foreach ($this->_elements as $id => $element)
        {
            /* @var $element OK_Form_Element */
            if (!$element->dataBinding()) continue;
            ...
            else
            {
                ...
                if ($nested_name || is_null($parent_data))
                {
                    ...
                    {
                        try { $value = $data[$id]; }
                        catch(OK_Exception $ex) { $value = null; }
                    }
                    else
                    {
