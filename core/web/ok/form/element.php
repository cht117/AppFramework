<?php
/**
 * 定义 OK_Form_Element 类
 *
 * @package form
 */

/**
 * OK_Form_Element 类封装了表单中的一个值元素
 *
 * @package form
 */
class OK_Form_Element
{
    /**
     * 属性值
     *
     ...
     *
     * @return OK_Form_Element 返回元素对象本身，实现连贯接口
     */
    function addFilters($filters)
    {
        if (!is_array($filters)) $filters = OK::normalize($filters);
    ...
    {
        $args = func_get_args();
        if ($validations instanceof OK_ActiveRecord_Meta)
        {
            if (isset($args[1]))
            {
     ...
     * 导入数据后进行过滤，并返回验证结果
     *
     * 通常调用 OK_Form 对象的 validate() 方法一次性导入整个表单的数据。
     *
    ...
    function validate($data, & $failed = null)
    {
        ...
        $data = Helper_Filter::filterBatch($data, $this->_filters);
        $this->_attrs['value'] = $data;

        if (!empty($this->_validations))
        {
            $failed = null;
            $this->_is_valid = (bool)OK_Validator::validateBatch($data, $this->_validations, OK_Validator::CHECK_ALL, $failed);
            if (!$this->_is_valid)
            {
