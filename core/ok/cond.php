...
class OK_Cond
{
    ...
    function formatToString($conn, $table_name = null, array $fields_mapping = null, $callback = null)
    {
        ...
        foreach ($this->_parts as $part)
        {
            ...
            if ($cond instanceof OK_Cond || $cond instanceof OK_Expr)
            {
                // 使用 OK_Cond 作为查询条件
            ...
            elseif (is_array($cond))
            {
                ...
                foreach ($cond as $field => $value)
                {
                    if (! is_string($field))
                    {
                        ...
                        if ($value instanceof OK_Cond || $cond instanceof OK_Expr)
                        {
                            // 查询条件如果是 OK_Cond 或 OK_Expr，则格式化为字符串
                        ...
                        $style = (strpos($value, '?') === false) ? OK_DB::PARAM_CL_NAMED : OK_DB::PARAM_QM;
            ...
            else
            {
                ...
                $style = (strpos($part, '?') === false) ? OK_DB::PARAM_CL_NAMED : OK_DB::PARAM_QM;
