...
class OK_Select
{
    ...
    const SQL_SUM = 'SUM';
    //查询虚拟字段
    const VIRTUAL_FIELD = 'VirtualField';
     ...
     */
    function __construct(OK_Adapter_Abstract $conn)
    {
        self::$_query_id ++;
        $this->_conn = $conn;
     ...
     * $cols 可以是一个以“,”分割的字段名字符串，也可以是一个数组，以及一个 OK_Expr 对象。
     *
     * 例如：
     *
     * @code php
     ...
     * 如果 $cols 是一个 OK_Expr 对象，则表达式中的字段名需要使用“[]”来指定转义。
     *
     * @code php
     * // SELECT LEFT(`posts`.`title`, 5) FROM `posts`
     * $expr = new OK_Expr('LEFT([title], 5)');
     ...
     *
     * @param  array|string|OK_Table $table
     * @param  array|string|OK_Expr $cols
     *
     * @return OK_Select
     */
    ...
    function setVirtualField($cols)
    {
        if (!empty($cols))
        {
        	 $this->_parts[self::VIRTUAL_FIELD][] = $cols;
        }
        return $this;
    }
     ...
     * @endcode
     *
     * 除了字符串和数组，$cond 参数还可以是 OK_Expr 对象，例如：
     *
     * @code php
     * $expr = new OK_Expr('[hits] < AVG([hits])');
     * $select->where($expr);
     * @endcode
     *
     * 如果没有在字段名中指定表名称或者别名，则假定所有字段都是第一个通过 from() 指定的表。
     *
     * 更复杂的查询条件，可以使用 OK_Cond 对象来构造。
     *
     * @param string|array|OK_Expr|OK_Cond $cond 查询条件
     *
     * @return OK_Select
     */
    function where($cond)
     ...
     *
     * @param OK_ActiveRecord_Association_Abstract|array $link
     *
     * @return OK_Select
     */
    function link($link)
    {
        ...
        {
            if ($link instanceof OK_ActiveRecord_Association_Abstract)
            {
                $this->_links[$link->mapping_name] = $link;
            }
            else
            {
                // LC_MSG: 关联必须是 OK_ActiveRecord_Association_Abstract 类型.
                throw new OK_Select_Exception(__('关联必须是 OK_ActiveRecord_Association_Abstract 类型.'));
            }
        }
     ...
     * @param  array|string|OK_Table $table
     * @param  array|string|OK_Expr $cols
     * @param  array|string|OK_Expr|OK_Cond $cond
     *
     * @return OK_Select
     */
     ...
     * 添加一个 UNION 查询
     *
     * $select 可以是一个字符串或一个 OK_Select 对象，或者包含上述两者的数组。
     *
     * @param  array|string|OK_Select $select
     *
     * @return OK_Select
     */
    function union($select = array(), $type = self::SQL_UNION)
    {
        ...
        if (! isset(self::$_union_types[$type]))
        {
            ...
            throw new OK_Select_Exception(__('无效的 UNION 类型 "%s".', $type));
        }
     ...
     * 指定 GROUP BY 子句
     *
     * $expr 可以是一个字符串或一个 OK_Expr 对象，或者包含上述两者的数组。
     *
     ...
     * @endcode
     *
     * @param string|OK_Expr|array $expr
     *
     * @return OK_Select
     */
    function group($expr)
     ...
     * 添加排序
     *
     * $expr 可以是字符串或者 OK_Expr 对象，例如：
     *
     * @code php
     * $select->order('title');
     * $select->order('users.username DESC');
     * $select->order(new OK_Expr('SUM(hits) ASC');
     * @endcode
     *
     * @param string $expr
     *
     ...
     *
     * fetchPagination() 方法可以让开发者更连贯的操作 OK_Select 对象。
     *
     * 例如：
     *
     * <code>
     ...
     *
     * 通常对于从 ActiveRecord 发起的查询不需要再调用该方法，oko 会确保此类查询都返回对象。
    ...
    {
        $this->_meta = OK_ActiveRecord_Meta::instance($class_name);
    ...
    /**
     * 指示将查询结果作为 OK_ActiveRecord_Association_Coll 集合返回
     *
     ...
     * OK_Select 会负责在生成查询时将映射名转换为实际的字段名。
     *
     * 例如：
     *
     * @code php
     ...
     *
     * @param OK_ActiveRecord_Association_Abstract $link
     *
     * @return OK_Select
     */
    function linkForRecursion(OK_ActiveRecord_Association_Abstract $link)
    {
    ...
    function getPart($part)
    {
        ...
        {
            // LC_MSG: 无效的部分名称 "%s".
            throw new OK_Select_Exception(__('无效的部分名称 "%s".', $part));
        }
    ...
    function getById($id,$included_links = null){
    	if ($this->_meta->idname_count !=1){
    		throw new OK_Select_Exception(__('getById 方法只适用于单一主键模型'));
    	}
     ...
     *
     * 默认情况下，OK_Select 对象会以数组形式返回查询结果。
     * 在这种模式下，关联的数据会被立即查询出来，并嵌入查询结果中。
     *
     * 如果指定 OK_Select 以 ActiveRecord 对象返回查询结果，则只有 $included_links 指定的关联会被立即查询。
    ...
    {
        $this->_query_params[self::NON_LAZY_QUERY] = OK::normalize($included_links);
    ...
    function __call($method, array $args)
    {
        if (strncasecmp($method,'get',3) === 0)
        {
            ...
            // support getByName getByNameAndSex etc.
            	...
            	if (count($keys) != count($args)){
            		throw new OK_Select_Exception(__('参数数量不对应'));
            	}
        ...
        // LC_MSG: OK_Select 没有实现魔法方法 "%s".
        throw new OK_NotImplementedException(__t('OK_Select 没有实现魔法方法 "%s".', $method));
    }
    ...
    protected function _queryArray($clean_up = true)
    {
        ...
        if ($this->_query_params[self::RECURSION] > 0 && ! empty($this->_used_links))
        {
            ...
            {
                /* @var $link OK_ActiveRecord_Association_Abstract */
                foreach ($keys as $key)
                {
                ...
                if ($link->type == OK_DB::MANY_TO_MANY)
                {
        ...
        if (count($this->_parts[self::AGGREGATE]) && isset($rowset))
        {
            if (empty($this->_parts[self::GROUP]))
            {
            	...
            	/**
            	 * 针对类似 amoeba 通过 count sum 操作,整合多条记录相加
            	 * 
            	 * 开始<<<
            	 */
            	if (count($rowset) > 1) {
            		...
            		$tmp_row = array();
            		$is_do_aggregate = array();
            		
            		foreach ( $this->_parts[self::AGGREGATE] as $val)
            		{
            			$is_do_aggregate[$val[2]] = $val[0];
            		}
            		
            		foreach ($rowset as $row_val)
            		{
            			foreach ($row_val as $key => $val)
            			{
            				if (isset($tmp_row[$key])){
            					if (!isset($is_do_aggregate[$key])) {
            						continue;
            					}         					
            					switch ($is_do_aggregate[$key])
            					{            						
            						case self::SQL_COUNT : $tmp_row[$key] += $val;
            							break;
            						case self::SQL_SUM : $tmp_row[$key] += $val;
            							break;
            						case self::SQL_MAX : if ($val > $tmp_row[$key]) $tmp_row[$key] = $val;
            							break;
            						case self::SQL_MIN : if ($val < $tmp_row[$key]) $tmp_row[$key] = $val;
            							break;
            						case self::SQL_AVG : $tmp_row[$key] = ($tmp_row[$key] + $val ) / 2;
            							break;
            						default:            							
            					}            					
            				}else {
     ...
     * 查询，并返回对象或对象集合
     *
     * @return OK_ActiveRecord_Association_Coll|OK_ActiveRecord_Abstract
     */
    protected function _queryObjects()
    {
        ...
        $no_lazy_query = OK::normalize($this->_query_params[self::NON_LAZY_QUERY]);
        while (($row = $handle->fetchRow()))
        {
            ...
            $obj = new $class_name($row, OK_DB::FIELD, true);
        ...
        if (empty($rowset))
        {
            ...
            else
            {
                ...
                {
                    return new OK_ActiveRecord_Association_Coll($this->_meta->class_name);
                }
                else
                {
                    return array();
        ...
        else
        {
            if ($this->_query_params[self::AS_COLL])
            {
                return OK_ActiveRecord_Association_Coll::createFromArray($rowset, $this->_meta->class_name);
            }
            else
            {
    ...
    protected function _renderColumns()
    {
        ...
        {
        	// 修正 COUNT 因 GROUP BY 的产生的错误数据
        	if (count($this->_parts[self::GROUP])>0)
        	{
        		return 'COUNT(DISTINCT '.implode(',', $this->_parts[self::GROUP]).' )';
        ...
        foreach ($this->_parts[self::COLUMNS] as $entry)
        {
            ...
            // $col 是一个字段名或者一个 OK_Expr 对象
            if ($col instanceof OK_Expr)
            {
                /* @var $col OK_Expr */
        ...
        // 如何使用了虚拟字段则添加一个查询字段
        if (!empty($this->_parts[self::VIRTUAL_FIELD])) {
        	foreach ($this->_parts[self::VIRTUAL_FIELD] as $fleld)
        	{
        		$columns[] = $fleld;
        	}
        }
    ...
    protected function _renderAggregate()
    {
        ...
        // 修正同时存在 Aggregate 与 Columns 时出现的 SQL 错误
        if (count($this->_parts[self::COLUMNS]) > 0 && !$this->_query_params[self::PAGED_QUERY])
        {
        	if (empty($columns)) 
        	{
        		$return = '';
    ...
    protected function _renderUnion()
    {
        ...
        if ($this->_parts[self::UNION])
        {
            ...
            foreach ($this->_parts[self::UNION] as $cnt => $union)
            {
                ...
                if ($target instanceof OK_Select)
                {
    ...
    {
    	// 如果是 getPagination 分页信息查询,则构造空的 GROUP 子句
    	if ($this->_query_params[self::PAGED_QUERY])
    	{
    		return '';
    	}
     ...
     *
     * @param int $join_type
     * @param array|string|OK_Table $name
     * @param array|string|OK_Expr $cols
     * @param array|string|OK_Expr|OK_Cond $cond
    ...
    protected function _join($join_type, $name, $cols, $cond = null, $cond_args = null)
    {
        if (! isset(self::$_join_types[$join_type]))
        {
            ...
            throw new OK_Select_Exception(__('无效的 JOIN 类型 "%s".', $join_type));
        }

        if (count($this->_parts[self::UNION]))
        {
            // LC_MSG: 不能在使用 UNION 查询的同时使用 JOIN 查询.
            throw new OK_Select_Exception(__('不能在使用 UNION 查询的同时使用 JOIN 查询.'));
        }
        ...
        elseif ($name instanceof OK_Table)
        {
            $table = $name;
            $alias = '';
        }
        ...
        // 处理查询条件
        if (! ($cond instanceof OK_Cond))
        {
            $cond = OK_Cond::createByArgs($cond, $cond_args);
        }
        /* @var $cond OK_Cond */
    ...
    protected function _addCols($table_name, $cols)
    {
        ...
        foreach ($cols as $alias => $col)
        {
            ...
            {
                // 将包含多个字段的字符串打散
                foreach (OK::normalize($col) as $col)
                {
     ...
     * 添加查询条件的内容方法
     *
     * @param string|array|OK_Expr|OK_Cond $cond
     * @param array $args
     * @param int $part_type
    ...
    protected function _addConditions($cond, array $args, $part_type, $bool)
    {
        if (!($cond instanceof OK_Cond))
        {
            ...
            $cond = OK_Cond::createByArgs($cond, $args);
        }

        if (is_null($this->_parts[$part_type]))
        {
            $this->_parts[$part_type] = new OK_Cond();
        }
        if ($bool)
        {
     ...
     * @param string|OK_Expr $field
     * @param string $alias
     * @param string $delcolumn TRUE
     *
     * @return OK_Select
     */
    protected function _addAggregate($type, $field, $alias, $delcolumn = TRUE)
    {
    	// 检查是否指示删除  Columns 
    	if ($delcolumn == TRUE)
    	{
    ...
    /**
     * 返回当前对象的一个复制品
     * 设计对子对象 OK_Cond Object 的关联, 所以需要 clone 一份 OK_Cond Object 
     * 
     *
     * @return OK_Select
     */
    function __clone()
    {
    	if (is_object($this->_parts['where'])) {
