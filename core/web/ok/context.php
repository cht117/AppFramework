...
/**
 * OK_Context 封装了运行时上下文
 *
 * 所谓运行时上下文是指 OKo 应用程序的运行环境本身。
 * OK_Context 主要封装了请求参数和请求状态，以及 URL 解析等功能。
 *
 * OK_Context 使用了单子设计模式，因此只能使用 OK_Context::instance() 来获得
 * OK_Context 对象的唯一实例。
 *
 * OK_Context 实现了 ArrayAccess 接口，可以将 OK_Context 对象当作数组一样使用。
 *
 * @code php
 ...
 * @package core
 */
class OK_Context implements ArrayAccess
{
    /**
     * 指示 UDI 中的部分
     */
    ...
    /**
     * 返回 OK_Context 对象的唯一实例
     *
     * @code php
     * $context = OK_Context::instance();
     * @endcode
     *
     * @return OK_Context OK_Context 对象的唯一实例
     */
    static function instance()
    {
        static $instance;
        if (is_null($instance)) $instance = new OK_Context();
        return $instance;
    }
     ...
     * @endcode
     *
     * 查找请求参数的顺行是 $_GET、$_POST 和 OK_Context 对象附加参数。
     *
     ...
     * 魔法方法，设置附加参数
     *
     * 与 __get() 魔法方法不同，__set() 仅仅设置 OK_Context 对象附加参数。
     * 因此当 $_GET 或 $_POST 中包含同名参数时，用 __set() 设置的参数值
     * 只能使用 OK_Context::param() 方法来获得。
     *
     * @code php
     * $context->title = $title;
     ...
     * 删除指定的附加参数
     *
     * __unset() 魔法方法只影响 OK_Context 对象的附加参数。
     *
     * @code php
     * unset($context['title']);
     ...
     *
     * 同 __unset() 方法，OK_Context::offsetUnset() 只影响 OK_Context 对象的附加参数。
     *
     * @code php
     * unset($context['title']);
     * @endcode
     ...
     * 魔法方法，访问请求参数
     *
     * OK_Context::get() 方法让开发者可以用 $context->parameter 的形式访问请求参数。
     ...
     * @param mixed $value 参数值
     *
     * @return OK_Context 返回 OK_Context 对象本身，实现连贯接口
     */
     ...
     * 设置 REQUEST_URI
     *
     * 这项修改不会影响 $_SERVER 中值，但是修改后 OK_Context::requestUri() 将返回新值。
     * 同时还影响 OK_Context::baseUri() 和 OK_Context::baseDir() 的返回结果。
     *
     ...
     * @code php
     * $udi = array(
     *     OK_Context::UDI_CONTROLLER => '',
     *     OK_Context::UDI_ACTION     => '',
     *     OK_Context::UDI_NAMESPACE  => '',
     *     OK_Context::UDI_MODULE     => '',
     * );
     *
     * // 输出
     * // array(
    ...
    /**
     * 将 OK_Context 对象保存的请求参数设置为 UDI 指定的值
     *
     * @code php
