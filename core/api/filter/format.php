<?php 
class API_Filter_Format extends API_Filter_Abstract
{
	private $charset;
	...
	function execute()
	{
		...
		switch ($format) 
		{
			...
			case 'xml':
				...
				$result = Helper_Array::toXml($result, $data_key);
				break;
			case 'php':
				@header("Content-Type: text/javascript;charset=utf-8");
				$result = serialize(array($data_key=>$result));
				break;
			case 'json':
				@header("Content-Type: text/javascript;charset=utf-8");
				$result = json_encode(array($data_key=>$result));
				break;
			case 'jsonp':
				//解决ajax跨域获取数据问题
				@header("Content-Type: text/javascript;charset=utf-8");
				...
				{
					$result = $_GET['jsoncallback'] .'('. json_encode(array($data_key=>$result)).')';
				}else {
					$result = '';
				}
