<?php
/**
 * 定义 OK_Adapter_Abstract 类
 *
 * @package database
 */

/**
 * OK_Adapter_Abstract 是所有数据库驱动的抽象基础类
 *
 * @package database
 */
abstract class OK_Adapter_Abstract
{
     ...
     * @var const
     */
    protected $_fetch_mode = OK_DB::FETCH_MODE_ASSOC;
     ...
     * @var string
     */
    protected $_param_style = OK_DB::PARAM_QM;
    ...
    function qid($name, $alias = null, $as = null)
    {
        ...
        {
        	/**
        	 * 修正别名不使用 identifier 方法 by fidoo 2011.1.10
    ...
    function qinto($sql, array $params = null, $param_style = null, $return_parameters_count = false)
    {
        ...
        {
        case OK_DB::PARAM_QM:
        case OK_DB::PARAM_DL_SEQUENCE:
            if ($param_style == OK_DB::PARAM_QM)
            {
                $parts = explode('?', $sql);
            }
            else
			...
			if (count($parts) > 2 and count($params) == 1) {
				$params = $params[0];
			}
            $str = $parts[0];
            $offset = 1;
        ...
        case OK_DB::PARAM_CL_NAMED:
        case OK_DB::PARAM_AT_NAMED:
            $split = ($param_style == OK_DB::PARAM_CL_NAMED) ? ':' : '@';
            $parts = preg_split('/(' . $split . '[a-z0-9_\-]+)/i', $sql, - 1, PREG_SPLIT_DELIM_CAPTURE);
            $max = count($parts);
            $str = $parts[0];
			if(count($params) == 1 and $max > 2) {
	    		$params = $params[0];
			}
            for ($offset = 1; $offset < $max; $offset += 2)
            {
                ...
                {
                    throw new OK_DB_Exception($sql, __('Invalid parameter "%s" for "%s"', $arg_name, $sql), 0);
                }
    ...
    function update($table_name, $row, array $where = null, array $restricted_fields = null)
    {
        ...
        if (!is_array($row) && !($row instanceof OK_Expr))
        {
            throw new OK_DB_Exception(__('Type mismatch'));
        }
    ...
    protected function _parseSQLString($table_name, $where, array $args)
    {
        ...
        {
            // 使用 ? 作为占位符的情况
            $ret = $this->qinto($where, $args, OK_DB::PARAM_QM, true);
        }
        elseif (strpos($where, ':') !== false)
        {
            // 使用 : 开头的命名参数占位符
            $ret = $this->qinto($where, reset($args), OK_DB::PARAM_CL_NAMED, true);
        }
        else
        {
            $ret = $where;
    ...
    static function getLogEnabled()
    {
    	...
    	if ($logEnabled == null)
    	{
    		if (OK::ini('db_log_enabled') && ok::ini('db_log_sampling') && rand(1, ok::ini('db_log_sampling')) == 1)
	        {
	        	$logEnabled = true;
	        }else {
	        	$logEnabled = false;
