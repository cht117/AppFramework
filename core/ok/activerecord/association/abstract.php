<?php
/**
 * 定义 OK_ActiveRecord_Association_Abstract 类
 *
 * @package orm
 */

/**
 * OK_ActiveRecord_Association_Abstract 封装 ActiveRecord 之间的关联关系
 *
 * @package orm
 */
abstract class OK_ActiveRecord_Association_Abstract
{
    /**
     ...
     * @param int $type
     * @param array $config
     * @param OK_ActiveRecord_Meta $source_meta
     *
     * @return OK_ActiveRecord_Association_Abstract
     */
    protected function __construct($type, array $config, OK_ActiveRecord_Meta $source_meta)
    {
        $this->type = $type;

        #IFDEF DEBUG
        OK_Log::log(__METHOD__ . "({$config['mapping_name']})", OK_Log::DEBUG);
        #ENDIF
     ...
     * @return OK_ActiveRecord_Association_Abstract
     */
    static function create($type, array $config, OK_ActiveRecord_Meta $source_meta)
    {
        if (empty($config['mapping_name']))
        {
            // LC_MSG: 创建关联必须指定关联的 mapping_name 属性.
            throw new OK_ActiveRecord_Association_Exception(__('创建关联必须指定关联的 mapping_name 属性.'));
        }
        else
        {
        ...
        {
        case OK_DB::HAS_ONE:
            return new OK_ActiveRecord_Association_HasOne(OK_DB::HAS_ONE, $config, $source_meta);
        case OK_DB::HAS_MANY:
            return new OK_ActiveRecord_Association_HasMany(OK_DB::HAS_MANY, $config, $source_meta);
        case OK_DB::BELONGS_TO:
            return new OK_ActiveRecord_Association_BelongsTo(OK_DB::BELONGS_TO, $config, $source_meta);
        case OK_DB::MANY_TO_MANY:
            return new OK_ActiveRecord_Association_ManyToMany(OK_DB::MANY_TO_MANY, $config, $source_meta);
        default:
            // LC_MSG: 无效的关联类型 "%s".
            throw new OK_ActiveRecord_Association_Exception(__('无效的关联类型 "%s".', $type));
        }
    }
    ...
    function init()
    {
    	if (!$this->_inited)
    	{
            $this->target_meta = OK_ActiveRecord_Meta::instance($this->_init_config['target_class']);
     ...
     * @return OK_ActiveRecord_Association_Abstract
     */
    abstract function onSourceSave(OK_ActiveRecord_Abstract $source, $recursion);
     ...
     * @return OK_ActiveRecord_Association_Abstract
     */
    abstract function onSourceDestroy(OK_ActiveRecord_Abstract $source);
}
