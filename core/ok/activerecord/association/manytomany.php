<?php
/**
 * 定义 OK_ActiveRecord_Association_ManyToMany 类
 *
 * @package orm
 */

/**
 * OK_ActiveRecord_Association_ManyToMany 类封装 ActiveRecord 对象之间的 many to many 关联
 *
 * @package orm
 */
class OK_ActiveRecord_Association_ManyToMany extends OK_ActiveRecord_Association_Abstract
{
    ...
    function init()
    {
        ...
        if (empty($p['mid_class']))
        {
            ...
            $this->mid_table = new OK_Table(array('name' => $mid_table_name,'conn' => $this->source_meta->table->getConn()));
        }
        else
        {
            // 如果中间表作为实体，则由指定的 ActiveRecord 继承类负责处理中间表
            $this->mid_meta = OK_ActiveRecord_Meta::instance($p['mid_class']);
     ...
     *
     * @return OK_ActiveRecord_Association_ManyToMany
     */
    function bindRelatedObject(OK_ActiveRecord_Abstract $source, OK_ActiveRecord_Abstract $target)
    {
        $this->init();
     ...
     * @return OK_ActiveRecord_Association_ManyToMany
     */
    function unbindRelatedObject(OK_ActiveRecord_Abstract $source, OK_ActiveRecord_Abstract $target)
    {
    }
     ...
     */
    protected function _updateRelation(OK_ActiveRecord_Abstract $source, $recursion = 1)
    {
        $this->init();
     ...
     *
     * @param OK_ActiveRecord_Abstract $source
     * @param array|OK_Coll $targets
     * @param int $recursion
     *
     * @return OK_ActiveRecord_Association_ManyToMany
     */
    protected function _updateRelationByMeta(OK_ActiveRecord_Abstract $source, $targets, $recursion)
    {
        /**
         * 算法：
         *
     ...
     * @return OK_ActiveRecord_Association_ManyToMany
     */
    protected function _updateRelationByTable(OK_ActiveRecord_Abstract $source, $targets, $recursion)
    {
        // 取出现有的关联信息
        $conn = $this->mid_table->getConn();
