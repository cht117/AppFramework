...
class OK_Log
{
    ...
    function flush()
    {
        ...
        // 更新日志记录优先级
        $keys = OK::normalize(OK::ini('log_priorities'));
        $arr = array();
        foreach ($keys as $key)
        {
        ...
        // 确定日志写入目录
        $dir = realpath(OK::ini('log_writer_dir'));
        if ($dir === false || empty($dir))
        {
            $dir = realpath(OK::ini('runtime_cache_dir'));
            if ($dir === false || empty($dir))
            {
                ...
                {
                    throw new OK_Log_Exception(__('指定的日志文件保存目录不存在 "%s".', OK::ini('log_writer_dir')));
                }
            }
        }

        $filename = OK::ini('log_writer_filename');
        $this->_filename = rtrim($dir, '/\\') . DS . $filename;
        $chunk_size = intval(OK::ini('log_cache_chunk_size'));
        if ($chunk_size < 1)
        {
            $chunk_size = 64;
        }
