        ...
        if ($this->caching) {
            ...
            if (smarty_core_read_cache_file($_params, $this)) {
                ...
                if (!empty($this->_cache_info['cache_serials'])) {
                    $_params = array('results' => $_smarty_results,'tpl_file' => $resource_name,);
   ...
   /**
    ...
    function _include($filename, $once=false, $params=null)
    {
    	...
    	if(!@is_writable($filename)) {
    		$smarty_compile_code = call_user_func_array($this->compiled_handler_func, array('read',array('filename'=>$filename), &$this));
    		if(!empty($smarty_compile_code)) {
			    if (!($fd = @fopen($filename, 'wb'))) {
			            $smarty->trigger_error("problem writing temporary file '$filename'");
			    }
			    fwrite($fd, $smarty_compile_code);
			    fclose($fd);
    		}
    	}
    	*/
    ...
    /**#@-*/
    /*
     * 增加一个方法,当启动模板缓存后,进行检查本地编译文件是否正常
     */
    function _do_compile_resource($resource_name)
    {
        $_smarty_compile_path = $this->_get_compile_path($resource_name);
        $_cache_including = $this->_cache_including;
        $this->_cache_including = false;
