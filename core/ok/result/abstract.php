<?php
/**
 * 定义 OK_Result_Abstract 类
 *
 * @package database
 */

/**
 * OK_Result_Abstract 是封装查询结果对象的抽象基础类
 *
 * @package database
 */
abstract class OK_Result_Abstract
{
	/**
	 * 指示返回结果集的形式
	 *
	...
	function fetchCol($col = 0)
	{
		...
		$this->fetch_mode = OK_DB::FETCH_MODE_ARRAY;
		$cols = array();
		while (($row = $this->fetchRow())) {
			$cols[] = $row[$col];
		}
    ...
    function fetchObject($class_name, $return_first = false) {
        $objs = array();
        $is_ar = is_subclass_of($class_name, 'OK_ActiverRecord_Abstract');

        while (($row = $this->fetchRow())) {
            $obj = $is_ar
                 ? new $class_name($row, OK_DB::FIELD, true)
                 : new $class_name($row);
        ...
        return OK_Coll::createFromArray($objs, $class_name);
    }
}
