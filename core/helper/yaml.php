...
abstract class Helper_YAML
{
     ...
     *
     * @return array 解析结果
     * @throw OK_FileNotFoundException
     */
    static function loadCached($filename, array $replace = null, $cache_backend = null)
    {
        ...
        if (!is_file($filename))
        {
            throw new OK_FileNotFoundException($filename);
        }
        ...
        if (is_null($cache_backend))
        {
            ...
            {
                $cache_obj = OK::singleton(OK::ini('runtime_cache_backend'));
            }
            $cache = $cache_obj;
        }
        else
        {
            $cache = OK::singleton($cache_backend);
        }

        /* @var $cache OK_Cache_File */
    ...
    {
        $parser = OK::singleton('sfYamlParser');
    ...
    {
        $dumper = OK::singleton('sfYamlDumper');
        return "# <?php die(); ?>\n\n" . $dumper->dump($data, 1) . "\n";
    }
