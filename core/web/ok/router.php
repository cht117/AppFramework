...
class OK_Router
{
    ...
    protected $_udi_parts = array(
        OK_Context::UDI_MODULE     => OK_Context::UDI_DEFAULT_MODULE,
        OK_Context::UDI_NAMESPACE  => OK_Context::UDI_DEFAULT_NAMESPACE,
        OK_Context::UDI_CONTROLLER => OK_Context::UDI_DEFAULT_CONTROLLER,
        OK_Context::UDI_ACTION     => OK_Context::UDI_DEFAULT_ACTION,
    );
    ...
    function import(array $rules, $cache_id = null)
    {
        if ($cache_id)
        {
            $backend = OK::ini('runtime_cache_backend');
            $routes = OK::cache($cache_id, null, $backend);
        }
        ...
        if (!$cache_id || !$routes)
        {
            ...
            if ($cache_id)
            {
                OK::writeCache($cache_id, $routes,
                    array('life_time' => OK::ini('routes_cache_lifetime')),
                    $backend);
            }
        }
    ...
    protected function _prepareRegexRoute($route_name, array $rule)
    {
        ...
        if (preg_match($route['regex'], '') === false)
        {
            ...
            throw new OK_Router_InvalidRouteException($route_name, __($msg, $route_name, $rule['regex']));
        }
        ...
        if (count($m[1]) != count($rule['config']))
        {
            ...
            throw new OK_Router_InvalidRouteException($route_name, __($msg, $route_name, count($m[1]), count($rule['config'])));
        }
        ...
        foreach ($parts as $pos => $part)
        {
            if ($part{0} == $this->_var_prefix)
            {
                ...
                if ($use_static_optional && !isset($rule['defaults'][$varname]))
                {
                    ...
                    throw new OK_Router_InvalidRouteException($route_name, __($msg, $route_name, $varname));
                }
            ...
            else
            {
                ...
                else
                {
                    if ($use_static_optional)
                    {
                        ...
                        throw new OK_Router_InvalidRouteException($route_name, __($msg, $route_name, $part));
                    }
