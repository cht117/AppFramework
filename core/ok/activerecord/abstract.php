<?php
/**
 * 定义 OK_ActiveRecord_Abstract 类
 *
 * @package orm
 */

/**
 * OK_ActiveRecord_Abstract 类实现了 Active Record 模式
 *
 * @package orm
 */
abstract class OK_ActiveRecord_Abstract implements OK_ActiveRecord_Callbacks,
                                                    OK_ActiveRecord_Interface,
                                                    ArrayAccess
{
    /**
     * 对象所有属性的值
     ...
     */
    function __construct($data = null, $names_style = OK_DB::PROP, $from_storage = false)
    {
        $this->_class_name = get_class($this);
        if (! isset(self::$_meta[$this->_class_name]))
        {
            self::$_meta[$this->_class_name] = OK_ActiveRecord_Meta::instance($this->_class_name);
        }
        $meta = self::$_meta[$this->_class_name];
        /* @var $meta OK_ActiveRecord_Meta */
     ...
     *
     * @return OK_ActiveRecord_Abstract 连贯接口
     */
     ...
     *
     * @return OK_ActiveRecord_Abstract 复制的 ActiveRecord 对象
     */
    function __clone()
    {
        foreach(self::$_meta[$this->_class_name]->idname as $name)
    ...
    function reload()
    {
        ...
        $this->changeProps($row, OK_DB::FIELD, null, true);
    }
    ...
    function validate($mode = 'general')
    {
        ...
        if (!empty($error))
        {
            throw new OK_ActiveRecord_ValidateFailedException($error, $this);
        }
    ...
    function destroy()
    {
        ...
        if (empty($id))
        {
            throw new OK_ActiveRecord_DestroyWithoutIdException($this);
        }
     ...
     * @return OK_ActiveRecord_Abstract 连贯接口
     */
    function changeProps($arr, $names_style = OK_DB::PROP, $attr_accessible = null, $_from_storage = false, $_ignore_readonly = false)
    {
        $meta = self::$_meta[$this->_class_name];
        /* @var $meta OK_ActiveRecord_Meta */
        ...
        if ($attr_accessible)
        {
            $attr_accessible = array_flip(OK::normalize($attr_accessible));
        ...
        {
        	// 转换小写(目前由于数据库的命名不规范,强制统一使用小写属性命名)
        	$prop_name = strtolower($prop_name);
        	
            if ($names_style == OK_DB::FIELD)
            {
    ...
    function changed($props_name = null)
    {
        ...
        $props_name = OK::normalize($props_name);
     ...
     * @return array
     */
    function toArray($recursion = 99, $names_style = OK_DB::PROP)
    {
        $data = array();
        $meta = self::$_meta[$this->_class_name];
        /* @var $meta OK_ActiveRecord_Meta */
     ...
     * @return string
     */
    function toJSON($recursion = 99, $names_style = OK_DB::PROP)
    {
    ...
    function __get($prop_name)
    {
        if (!isset(self::$_meta[$this->_class_name]->props[$prop_name]))
        {
            throw new OK_ActiveRecord_UndefinedPropException($this->_class_name, $prop_name);
        }
        ...
        {
            // 如果属性是一个关联，并且没有值，则通过 OK_ActiveRecord_Meta::relatedObjects() 获得关联对象
    ...
    function __set($prop_name, $value)
    {
        ...
        if ($config['readonly'])
        {
            throw new OK_ActiveRecord_ChangingReadonlyPropException($this->_class_name, $prop_name);
        }
        ...
        {
            // 在指定关联对象时，要进行类型检查
            if ($config['assoc'] == OK_DB::HAS_ONE || $config['assoc'] == OK_DB::BELONGS_TO)
            {
                ...
                {
                    throw new OK_ActiveRecord_SettingPropTypeMismatchException($this->_class_name,
            ...
            else
            {
                if (is_array($value))
                {
                    $this->_props[$prop_name] = OK_Coll::createFromArray($value, $config['assoc_class']);
                }
                ...
                {
                    throw new OK_ActiveRecord_SettingPropTypeMismatchException($this->_class_name, $prop_name, 'Iterator', gettype($value));
                }
            }
    ...
    function __call($method, array $args)
    {
        ...
        throw new OK_ActiveRecord_CallToUndefinedMethodException($this->_class_name, $method);
    }
     ...
     * @return string
     */
    static function multiToJSON(array $objects, $recursion = 99, $names_style = OK_DB::PROP)
    {
        $arr = array();
    ...
    protected function _create($recursion = 99)
    {
        ...
        {
            /* @var $assoc OK_ActiveRecord_Association_BelongsTo */
            ...
            if (empty($this->_props[$mapping_name]))
            {
                if (empty($this->_props[$source_key]))
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new OK_ActiveRecord_ExpectsAssocPropException($this->_class_name, $mapping_name);
                        }
                    }
                }
            }
            else
            {
                ...
                /* @var $belongsto OK_ActiveRecord_Abstract */
        ...
        {
            if ($assoc->type == OK_DB::BELONGS_TO || !isset($this->_props[$prop]))
            {
                continue;
            }

            /* @var $assoc OK_ActiveRecord_Association_Abstract */
            ...
            {
                throw new OK_ActiveRecord_ExpectsAssocPropException($this->_class_name, $assoc->source_key);
            }
...
//dprint($save_data);
    ...
    protected function _replace($recursion = 99)
    {
        ...
        catch (OK_DB_Exception_DuplicateKey $ex)
        {
