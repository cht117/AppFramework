...
/**
 * OK_Coll 实现了一个类型安全的对象集合
 *
 * OK_Coll 会检查每一个对象的类型是否符合预期，以便将同一类型的对象组织在一起。
 * OK_Coll 具有和 PHP 内置数组相似的性质，因此可以按照使用数组的方式来使用 OK_Coll 集合。
 *
 * 在构造一个集合时，必须指定该集合能够容纳的对象类型：
 *
 * @code php
 * $coll = new OK_Coll('MyObject');
 * $coll[] = new MyObject();
 *
 ...
 * @package core
 */
class OK_Coll implements Iterator, ArrayAccess, Countable
{
    /**
     * 集合对象的类型
     *
     ...
     * 从数组创建一个集合
     *
     * OK_Coll::createFromArray() 方法从一个包含多个对象的数组创建集合。
    ...
    function offsetGet($offset)
    {
        ...
        // LC_MSG: 无效的键名 "%s".
        throw new OK_Exception(__('无效的键名 "%s".', $offset));
    }
    ...
    function first()
    {
        ...
        // LC_MSG: "%s" 集合中没有任何对象.
        throw new OK_Exception(__('"%s" 集合中没有任何对象.', $this->_type));
    }
     ...
     * @endcode
     *
     * OK_Coll::append() 在追加数据时不会保持键名。
     *
     * @param array|OK_Coll $data 要追加的数据
     *
     * @return OK_Coll 返回集合对象本身，实现连贯接口
     */
    function append($data)
    {
        ...
        else
        {
            ...
            throw new OK_Exception(__('"%s()" 的参数 "%s" 必须是 "%s"，但实际提供的是 "%s".',
     ...
     * @endcode
     *
     * 当调用 OK_Coll 自身没有定义的方法时，OK_Coll 将认为开发者是要对集合中的每一个对象调用指定方法。
     *
     * -  此时，OK_Coll 首先检查集合中的对象是否提供了 _qcoll_callback() 静态方法；
     * -  如果有，则通过 _qcoll_callback() 取得一个方法映射表；
     * -  OK_Coll 根据 _qcoll_callback() 返回的方法映射表调用对象的其他静态方法。
     * -  如果没有提供 _qcoll_callback() 方法，或方法映射表中没有指定的方法。
     *    OK_Coll 则遍历集合中的所有对象，尝试调用对象的指定方法。
     *
     * @param string $method
     * @param array $args;
     *
    ...
    protected function _checkType($object)
    {
        ...
        throw new OK_Exception(__('集合只能容纳 "%s" 类型的对象，而不是 "%s" 类型的值.', $this->_type, $type));
    }
