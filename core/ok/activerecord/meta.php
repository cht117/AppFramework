<?php
/**
 * 定义 OK_ActiveRecord_Meta 类
 *
 * @package orm
 */

/**
 * OK_ActiveRecord_Meta 类封装了 OK_ActiveRecord_Abstract 继承类的元信息
 *
 * @package orm
 */
class OK_ActiveRecord_Meta implements OK_ActiveRecord_Callbacks
{
	/**
	 * ID 属性名
	 *
	 ...
	 * @endcode
	 *
	 * 如果关联已经初始化，则 $assoc 是一个 OK_ActiveRecord_Association_Abstract 继承类实例。
	 * 否则 $assoc 为 false。
	 *
	 * @var array of OK_ActiveRecord_Association_Abstract
	 */
	 ...
	 * 行为插件对象
	 *
	 * @var array of OK_ActiveRecord_Behavior_Abstract objects
	 */
	 ...
	 *
	 * @var array of OK_ActiveRecord_Meta
	 */
	...
	static function instance($class)
	{
		...
		{
			self::$_metas[$class] = new OK_ActiveRecord_Meta($class);
	 ...
	 *
	 * @return OK_ActiveRecord_Abstract
	 */
	...
	{
		$select = new OK_Select($this->table->getConn());
	...
	function validate(array $data, $props = null, $mode = 'general')
	{
		if (!is_null($props))
		{
			$props = array_flip(OK::normalize($props));
		}
		else
		{
		...
		foreach ($this->validations as $prop => $policy)
		{
			...
			if (isset($policy[$mode]))
			{
				...
				// $__tmp = $policy['rules'];
				// $policy = $policy[$mode];
				// $policy['rules'] = array_merge($policy['rules'] , $__tmp);
				// unset($__tmp);
			}
			...
			foreach ($policy['rules'] as $index => $rule)
			{
				...
				$ret = OK_Validator::validateByArgs($validation, $rule);
				if ($ret === OK_Validator::SKIP_OTHERS)
				{
					break;
				}
				elseif (!$ret)
	 ...
	 *
	 * @return OK_ActiveRecord_Abstract|OK_ActiveRecord_Association_Coll
	 */
	function relatedObjects(OK_ActiveRecord_Abstract $obj, $prop_name)
	{
		/**
		...
		if (empty($source_key_value))
		{
			...
			{
				return new OK_ActiveRecord_Association_Coll($target_meta->class_name);
			}
		}
		...
		case OK_DB::MANY_TO_MANY:
			...
			/* @var $assoc OK_ActiveRecord_Association_ManyToMany */
		...
		default:
			// LC_MSG: 无效的关联类型 "%s".
			throw new OK_ActiveRecord_Association_Exception(__('无效的关联类型 "%s".', $assoc->type));
		}

		/* @var $select OK_Select */
	...
	{
		$behaviors = OK::normalize($behaviors);
		if (!is_array($config))
		{
			$config = array();
		}
	...
	function addDynamicMethod($method_name, $callback, array $custom_parameters = array())
	{
		if (!empty($this->methods[$method_name]))
		{
			...
			throw new OK_ActiveRecord_Meta_Exception(__('指定的动态方法名 "%s" 已经存在于 "%s" 对象中.', $method_name, $this->class_name));
		}
	...
	function addStaticMethod($method_name, $callback, array $custom_parameters = array())
	{
		if (!empty($this->static_methods[$method_name]))
		{
			...
			throw new OK_ActiveRecord_Meta_Exception(__('指定的静态方法名 "%s" 已经存在于 "%s" 对象中.', $method_name, $this->class_name));
		}
	 ...
	 * @return OK_ActiveRecord_Meta
	 */
	function addExceptionTrap(OK_ActiveRecord_Abstract $obj, $exception_type, $callback, array $custom_parameters = array())
	{
	...
	function addProp($prop_name, array $config)
	{
		if (isset($this->props[$prop_name]))
		{
			...
			throw new OK_ActiveRecord_Meta_Exception(__('尝试添加的属性 "%s" 已经存在.', $prop_name));
		}
	 ...
	 *
	 * $assoc_type 指定了关联的类型，可以是 OK_DB::BELONGS_TO、OK_DB::HAS_MANY、OK_DB::HAS_ONE 或 OK_DB::MANY_TO_MANY。
	 *
	...
	function addAssoc($prop_name, $assoc_type, array $config)
	{
		...
		$association = OK_ActiveRecord_Association_Abstract::create($assoc_type, $assoc, $this);
		$association->registerCallbacks($assoc);
		$this->associations[$prop_name] = $association;

		if ($association->type == OK_DB::BELONGS_TO)
		{
			$association->init();
	...
	function assoc($prop_name)
	{
		...
		{
			throw new OK_ActiveRecord_Association_NotDefinedException($this->class_name, $prop_name);
		}
	...
	function __call($method_name, array $args)
	{
		...
		throw new OK_ActiveRecord_Meta_Exception(__('未定义的方法 "%s".', $method_name));
	}
	...
	protected function _prepareValidationRules($policies, array $ref = array(), $set_policy = true)
	{
		...
		foreach ($policies as $prop_name => $policy)
		{
			...
			foreach ($policy as $option => $rule)
			{
				...
				{
					// LC_MSG: 指定了无效的验证规则 "%s".
					throw new OK_ActiveRecord_Meta_Exception(__('指定了无效的验证规则 "%s".', $option . ' - ' . $rule));
				}
			}
		}
	...
	{
		$obj_id = 'activerecord_table_' . strtolower($table_name).'_'.strtolower($table_config['dsn']);
		if (OK::isRegistered($obj_id))
		{
			return OK::registry($obj_id);
		}
		else
		{
			$table_config['name'] = $table_name;
			$table = new OK_Table($table_config);
			OK::register($table, $obj_id);
			return $table;
		}
	}
	...
	/**
	 * 执行一个绑定 SQL 语句
	 * - 如果是一个 SELECT 操作 则返回一个查询结果 array
	 * - 如果是一个 NOSELECT 操作 则返回是否成功
	 *
	 * @param string $sql_name 绑定的SQL语句名称
	 * @return boolean | array
	 */
	function dbTool($sql_name)
	{
		// 获取 dbtool 路径
		$filename = OK::ini('app_config/APP_DIR') .DS. 'dbtool' .DS. 'sql_'.strtolower($this->class_name).'.'.OK::ini('app_config/CONFIG_FILE_EXTNAME');
		if (! file_exists($filename)) {
			return null;
		}
		// 加载SQL
		$contents = Helper_YAML::load($filename);
		// 检查符合要求的SQL
		if (empty($contents[$sql_name]['SQL'])) {
			return null;
		}
		// 构造参数
		$args = func_get_args();
		if (empty($contents[$sql_name]['MODE']) || $contents[$sql_name]['MODE'] != 'NOSELECT') {
			$args[0] = true;
		}else {
			$args[0] = false;
		}
		
		return $this->table->dbTool($contents[$sql_name]['SQL'] , $args);
	...
	private function _init1($class)
	{
		...
		{
			$this->create_reject = array_flip(OK::normalize($ref['create_reject']));
		}
		if (!empty($ref['update_reject']))
		{
			$this->update_reject = array_flip(OK::normalize($ref['update_reject']));
		}
		...
		{
			$this->attr_accessible = array_flip(OK::normalize($ref['attr_accessible']));
		}
		if (!empty($ref['attr_protected']))
		{
			$this->attr_protected = array_flip(OK::normalize($ref['attr_protected']));
		}
