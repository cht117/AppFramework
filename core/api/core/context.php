<?php
/**
 * 定义 API_Core_Context 类
 *
 * @package core
 */

/**
 * API_Core_Context 封装了运行时上下文
 *
 * 所谓运行时上下文是指  API 封装系统 应用程序的运行环境本身。
 * API_Core_Context 主要封装了请求参数和请求状态，以及 URL 解析等功能。
 *
 * API_Core_Context 使用了单子设计模式，因此只能使用 API_Core_Context::instance() 来获得
 * API_Core_Context 对象的唯一实例。
 *
 * API_Core_Context 实现了 ArrayAccess 接口，可以将 API_Core_Context 对象当作数组一样使用。
 *
 * @code php
 ...
 * @package core
 */
class API_Core_Context implements ArrayAccess
{
    /**
     * 指示 UDI 中的部分
     */
    ...
    // 默认的模块
    const UDI_DEFAULT_MODULE     = 'module';
    // 默认的名字空间
    ...
    /**
     * 根据服务器运行环境，重新初始化 API_Core_Context 对象
     *
    ...
    function reinit($full_init = false)
    {
        ...
        // 如果有必要，初始化路由服务
        $url_mode = strtolower(OK::ini('dispatcher_url_mode'));
        if (is_null($this->_router)
            && ($url_mode == self::URL_MODE_PATHINFO || $url_mode == self::URL_MODE_REWRITE))
        {
            $this->_router = new OK_Router();
            
            $this->_router->import(OK::ini('routes'), 'global_default_routes');
        ...
        if (isset($_GET[$keys[self::UDI_METHOD]])) {
        	$method = explode('.', $_GET[$keys[self::UDI_METHOD]]);
        	switch (count($method))
        	{
        		case 2 : $udi = $this->MethodToUDI($method, array(self::UDI_CONTROLLER,self::UDI_ACTION));
        			break;
        		case 3 : $udi = $this->MethodToUDI($method, array(self::UDI_MODULE,self::UDI_CONTROLLER,self::UDI_ACTION));
        			break;
        		case 4 : $udi = $this->MethodToUDI($method, array(self::UDI_MODULE,self::UDI_NAMESPACE,self::UDI_CONTROLLER,self::UDI_ACTION));
        			break;
        		default: $udi = array();
        	}
        }else {
    ...
    private function MethodToUDI($method , $method_key)
    {
    	$udi = array();
    	foreach ($method_key as $value)
    	{
    		$udi[$value] = array_shift($method);
    	}
    ...
    /**
     * 返回 API_Core_Context 对象的唯一实例
     *
     * @code php
     * $context = API_Core_Context::instance();
     * @endcode
     *
     * @return API_Core_Context API_Core_Context 对象的唯一实例
     */
    static function instance()
    {
        static $instance;
        if (is_null($instance)) $instance = new API_Core_Context();
        return $instance;
    }
     ...
     *
     * 同 __unset() 方法，API_Core_Context::offsetUnset() 只影响 API_Core_Context 对象的附加参数。
     *
     * @code php
     * unset($context['title']);
     * @endcode
    ...
    function system($parameter = null, $default = null)
    {
    	if (is_null($parameter))
    		return $this->_SYSTEM;
    	return isset($this->_SYSTEM[$parameter]) ? $this->_SYSTEM[$parameter] : $default;
    }
    
    function setSystem($parameter, $value)
    {
    	$this->_SYSTEM[$parameter] = $value;
    }
     ...
     * 设置 REQUEST_URI
     *
     * 这项修改不会影响 $_SERVER 中值，但是修改后 API_Core_Context::requestUri() 将返回新值。
     * 同时还影响 API_Core_Context::baseUri() 和 API_Core_Context::baseDir() 的返回结果。
     *
     ...
     * @endcode
     *
     * 在使用 PATHINFO 和 URL 重写时，可以使用通过制定路由名来强制要求 oko
     ...
     * @code php
     * $udi = array(
     *     API_Core_Context::UDI_CONTROLLER => '',
     *     API_Core_Context::UDI_ACTION     => '',
     *     API_Core_Context::UDI_NAMESPACE  => '',
     *     API_Core_Context::UDI_MODULE     => '',
     * );
     *
     * // 输出
     * // array(
    ...
    /**
     * 将 API_Core_Context 对象保存的请求参数设置为 UDI 指定的值
     *
     * @code php
