 ...
 * 然后由 OK_ACL 比对角色组和 ACL，并返回检查结果。
 *
 * OK_ACL::rolesBasedCheck() 用于比对权限，并返回结果。
 * OK_ACL::normalize() 方法用于将 ACL 转换为符合规范的 ACL。
 *
 */
class OK_ACL
{
    ...
    {
        $roles = array_map('strtolower', OK::normalize($roles));
        ...
        if ($acl['allow'] == self::ACL_EVERYONE)
        {
            ...
            {
                throw new OK_ACL_Exception('Invalid acl');
            }
