...
/**
 * 类 Helper_Filter 实现了过滤器接口，以及一些常用的过滤器方法
 *
 * Helper_Filter 的所有方法都是静态方法，可以采用三种方式使用 Helper_Filter：
 *
 * -   直接调用特定的过滤方法对值进行过滤
 *
 *     @code php
 *     $value = Helper_Filter::filter_alpha($value);
 *     @endcode
 *
 * -   通过 Helper_Filter::filter() 调用过滤器
 *     @code php
 *     $value = Helper_Filter::filter($value, 'alpha');
 *     @endcode
 *
 * -   通过 Helper_Filter::filterBatch() 批量调用过滤器
 *     @code php
 *     $value = Helper_Filter::filterBatch($value, $filters);
 *     @endcode
 *
...
abstract class Helper_Filter
{
     ...
     *
     * Helper_Filter 内置的过滤器，实际上是 Helper_Filter 类中以“filter_”开头的静态方法。
     * 例如：filter_alpha、filter_digits。
     *
     * 大多数过滤器只需要一个参数 ，及要过滤的值。但部分过滤器要求更多的参数。
     * 此时在调用 Helper_Filter::filter() 时，可以将额外的参数一起提供：
     *
     * @code php
     * // 等同于 $value = substr($value, 0, -3)
     * $value = Helper_Filter::filter($value, 'substr', 0, -3);
     * @endcode
     *
     ...
     * 用一组过滤器过滤值，返回过滤结果
     *
     * Helper_Filter::filterBatch() 可以很方便对一个值使用一组过滤器，并返回最终结果。
     *
     * @code php
     * $filters = array(
     *     array('alpha'),
    ...
    static function filterByArgs($value, array $args)
    {
        ...
        throw new OK_NotImplementedException(__($filter));
    }
     ...
     * @code php
     * $value = 'abcd."1234"';
     * $value = Helper_Filter::filter_alnum($value);
     * // 过滤后的值为 abcd1234
     * @endcode
     *
     * 除了直接调用该方法，还可以通过 Helper_Filter::filter() 等方法来调用这个过滤器。
     *
